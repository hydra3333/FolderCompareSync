...
In Python, modules are singletons per process â€" they live ONCE in sys.modules.
...
If the module has not been loaded yet, itâ€™s imported and inserted into sys.modules["FolderCompareSync_Global_Constants"].
Module A then binds its local name C to that single module object.

When module B later does the same import:
    import FolderCompareSync_Global_Constants as C
Python checks sys.modules, sees itâ€™s already loaded, and reuses the same module object.
...
What wouldnâ€™t see changes is if module D instead did:
    from FolderCompareSync_Global_Constants import STATUS_LOG_MAX_HISTORY
Using "from ... import ..." COPIES the value into module Dâ€™s local globals at import time, so 
...
Summary:
1. "import â€¦ as C"    ->  shared live module object (safe for on-the-fly editing).
2. "from â€¦ import X"  ->  copies the value at import time (edits wonâ€™t propagate).
