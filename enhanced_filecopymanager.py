...
class FileCopyManager_class:
    ...
    class CopyStrategy(Enum):
        ...
        """
        DIRECT = "direct"      # Strategy A: CopyFileExW + mmap verification for local files <2GB
        STAGED = "staged"      # Strategy B: Chunked I/O + BLAKE3 hashing for network/large files
        ...
        try:
            ...
            with open(temp_path, 'rb') as temp_file:
                while True:
                    ...
                    # # DEBUG: Uncomment for detailed hash verification tracking  
                    # if bytes_processed % (chunk_size * 10) == 0:  # Every 10 chunks
                    #     log_and_flush(logging.DEBUG, f"Hash verification progress: {bytes_processed:,} bytes")
    ...
    def _check_sufficient_disk_space(self, source_path: str, target_path: str) -> bool:
        ...
        try:
            ...
            if Path(target_path).exists():
                space_needed += Path(target_path).stat().st_size  # For backup
